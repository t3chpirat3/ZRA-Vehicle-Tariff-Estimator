...
type VehicleAge = '0-2' | '2-5' | '5+';
type VehicleCategory = 'motor-car' | 'goods-vehicle' | 'bus' | 'motorcycle';

type MotorCarType = 'sedan' | 'hatchback' | 'station' | 'suv';
type GoodsVehicleType = 'single-cab' | 'double-cab' | 'panel-van' | 'truck';
type BusFuelType = 'diesel' | 'other-diesel';

type FuelType = 'petrol' | 'diesel' | 'hybrid' | 'electric';

type VehicleOrigin = 'Japan' | 'UK' | 'Singapore' | 'South Africa' | 'Thailand' | 'Other' | '';
 ...
 */
const T_HYBRID_MOTOR_CAR_RATES: Record<
  '2-5' | '5+',
...
function zmwFormat(num: number): string {
...
function getCarbonBand(cc: number): string {
...
function getWeightBand(w: string, type: GoodsVehicleType): string {
...
function getBusSeatBand(seatsStr: string): string {
...
function calculateDuty(state: CalculatorState, overrides?: TaxRatesOverride): CalculationResult | null {
...
const WEIGHT_OPTIONS_MAP: Record<GoodsVehicleType, { v: string; l: string }[]> = {
  'single-cab': [
...
const calc = { age: '0-2', cat: 'goods-vehicle', fuel: 'diesel', hpCC: '', hpHP: '', cifUSD: 756000, fx: 26.5, cifEngine: '2393' };
console.log('if cifUSD=756000:', calculateDuty(calc));

const calc2 = { ...calc, cifUSD: 28528.30 };
console.log('if cifUSD=28528.30:', calculateDuty(calc2));
